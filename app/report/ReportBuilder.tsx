...
const SECTIONS: SectionDef[] = [
  ...
  { id: 'nadiChakra', label: 'நாடி சக்கரம்' },
  ...
  { id: 'reportRefinements', label: 'Report & Export (PDF, Image, HTML)' },
...
function NadiChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const NadiChakraRenderer = require('@/src/charts/chart-renderers/NadiChakraRenderer').NadiChakraRenderer;
  const nadi = (report as any).nadiChakra;
  if (!nadi) return null;
  return <NadiChakraRenderer data={nadi} />;
}

function DashaChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const DashaChakraRenderer = require('@/src/charts/chart-renderers/DashaChakraRenderer').DashaChakraRenderer;
  const dasha = (report as any).dashaChakra;
  if (!dasha) return null;
  return <DashaChakraRenderer data={dasha} />;
}

function RashiChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const RashiChakraRenderer = require('@/src/charts/chart-renderers/RashiChakraRenderer').RashiChakraRenderer;
  const rashi = (report as any).rashiChakra;
  if (!rashi) return null;
  return <RashiChakraRenderer data={rashi} />;
}

function YantraChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const YantraChakraRenderer = require('@/src/charts/chart-renderers/YantraChakraRenderer').YantraChakraRenderer;
...
function BhavaChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const BhavaChakraRenderer = require('@/src/charts/chart-renderers/BhavaChakraRenderer').BhavaChakraRenderer;
  const bhava = (report as any).bhavaChakra;
  if (!bhava) return null;
  return <BhavaChakraRenderer data={bhava} />;
}

function ElementalChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const ElementalChakraRenderer = require('@/src/charts/chart-renderers/ElementalChakraRenderer').ElementalChakraRenderer;
  const elemental = (report as any).elementalChakra;
  if (!elemental) return null;
  return <ElementalChakraRenderer data={elemental} />;
}

function TransitChakraSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const TransitChakraRenderer = require('@/src/charts/chart-renderers/TransitChakraRenderer').TransitChakraRenderer;
...
function ReportRefinementsSection({ report }: { report: ReportData }) {
  // Dynamically import the renderer to avoid circular dependencies
  const ReportRefinementsRenderer = require('@/src/charts/chart-renderers/ReportRefinementsRenderer').ReportRefinementsRenderer;
  const refinements = (report as any).reportRefinements;
  if (!refinements) return null;
  return <ReportRefinementsRenderer data={refinements} />;
}
...
const SECTION_RENDERERS: Record<string, React.ComponentType<{ report: ReportData }>> = {
  ...
  reportRefinements: ReportRefinementsSection,
